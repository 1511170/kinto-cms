...
async function main() {
  console.log('Step 1: Exchanging code for admin token...');
  ...
  if (exchanged.status < 200 || exchanged.status >= 300 || !exchanged.body?.access_token) {
    console.error('Error:', exchanged.text);
    process.exit(1);
  }
  ...
  console.log('\nStep 2: Creating storefront token...');
  ...
  if (created.status < 200 || created.status >= 300 || !created.body?.storefront_access_token?.access_token) {
    console.error('Error:', created.text);
    process.exit(1);
  }
  ...
  console.log('\nTokens saved to .env!');
  console.log('Storefront token prefix:', storefrontToken.substring(0, 20) + '...');
}
