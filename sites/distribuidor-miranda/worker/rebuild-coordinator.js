...
import { mergeRebuildEvent, parseDebounceWindowMs, } from "./utils/rebuild-coordinator";
const PENDING_REBUILD_KEY = "pending-rebuild";
export class ShopifyRebuildCoordinator extends DurableObject {
    async queue(event) {
        const now = Date.now();
        const pending = (await this.ctx.storage.get(PENDING_REBUILD_KEY)) ?? null;
        const merged = mergeRebuildEvent(pending, event, now);
        const flushAt = now + parseDebounceWindowMs(this.env.SHOPIFY_REBUILD_DEBOUNCE_SECONDS);
    ...
    async alarm() {
        const pending = await this.ctx.storage.get(PENDING_REBUILD_KEY);
        if (!pending)
            return;
        ...
        const ok = await triggerDeployHook(this.env.DEPLOY_HOOK_URL, this.env.GITHUB_DEPLOY_TOKEN);
        if (!ok) {
