...
export function parseDebounceWindowMs(value) {
...
export function mergeRebuildEvent(pending, event, now) {
    if (!pending) {
        return {
            eventCount: 1,
            firstQueuedAt: now,
    ...
    return {
        ...
        sampleResourceHandles: addSample(pending.sampleResourceHandles, event.resourceHandle),
