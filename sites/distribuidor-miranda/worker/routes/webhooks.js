...
async function verifyWebhookSignature(request, secret) {
    ...
    const secretCandidates = [encoder.encode(secret)];
...
async function queueRebuild(env, event) {
...
export async function handleWebhook(request, env, ctx, _pathSegments) {
    if (request.method !== "POST") {
        return apiError("Method not allowed", "METHOD_NOT_ALLOWED", 405);
    }
    // ── Verify HMAC signature ──────────────────────────────────────────────────
    const valid = await verifyWebhookSignature(request, env.WEBHOOK_SIGNING_SECRET);
    if (!valid) {
    ...
    const resourceHandle = body.handle;
    ...
    const shouldRebuild = Boolean(env.DEPLOY_HOOK_URL && REBUILD_TOPICS.has(topic));
    const rebuildMode = shouldRebuild && env.SHOPIFY_REBUILD_COORDINATOR
        ? "debounced"
        : shouldRebuild
            ? "direct"
            : "none";
    ...
    if (shouldRebuild) {
        ctx.waitUntil(queueRebuild(env, {
            topic,
            webhookId,
            resourceHandle: resourceHandle ?? null,
