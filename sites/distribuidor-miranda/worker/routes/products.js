...
import { ALL_PRODUCTS_QUERY, PRODUCT_BY_HANDLE_QUERY, } from "../../config/shopify.graphql";
const PRODUCT_TTL = 3600; // 1 hour
export async function handleProducts(request, env, pathSegments) {
    ...
    try {
        ...
        if (handle) {
            ...
            const result = await shopifyRequest(env, PRODUCT_BY_HANDLE_QUERY, {
                handle,
            });
            if (!result.ok) {
        ...
        const result = await shopifyPaginate(env, ALL_PRODUCTS_QUERY, (data) => data.products, {}, 250);
        if (!result.ok) {
            return apiError(result.errors.join("; "), "SHOPIFY_API_ERROR", 502);
        }
        const products = (result.data ?? []).map((node) => mapShopifyProduct(node));
        // Fire-and-forget cache write
        setCache(env.SHOPIFY_CACHE, listCacheKey, products, PRODUCT_TTL);
        return apiSuccess({ products });
    }
    catch (err) {
        return apiError(err?.message ?? "Failed to fetch products", "INTERNAL_ERROR", 500);
    }
}
