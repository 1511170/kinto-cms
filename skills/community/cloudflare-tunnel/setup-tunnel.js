...
function decodeToken(token) {
  try {
    ...
    fatal(
      "Invalid JWT token. Make sure you copied the full token from Cloudflare Dashboard.",
    );
  }
}
...
function checkServer() {
  try {
    execSync(
      `curl -s -o /dev/null -w "%{http_code}" http://localhost:${CONFIG.port}/`,
      { timeout: 5000 },
    );
    success(`Local server responding on port ${CONFIG.port}`);
  } catch {
    warning(
      `Server not responding on port ${CONFIG.port}. Tunnel will start but won't serve content until the server is running.`,
    );
  }
}
...
function configureDNS(cloudflaredBin, tunnelId, hostname) {
  ...
  try {
    ...
    if (msg.includes("already exists")) {
      ...
      info(
        "Configure manually in Cloudflare Dashboard: Zero Trust > Networks > Tunnels",
      );
    }
  }
}
...
function verifyTunnel(hostname, maxAttempts = 12) {
  ...
  info(
    "The tunnel may need 1-2 more minutes. Check logs with: tail -f /tmp/tunnel-*.log",
  );
  return false;
}
...
function createStartScript(cloudflaredBin, prodTunnelId, devTunnelId) {
  ...
  const stopCommands = stopIds
    .map((id) => `pkill -f "cloudflared.*${id}" 2>/dev/null`)
    .join("\n");
...
async function main() {
  ...
  if (devConfig) {
    if (devTunnelId !== prodTunnelId)
      log(`    ~/.cloudflared/${devTunnelId}.json`);
