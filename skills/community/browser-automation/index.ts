...
const skill: Skill = {
  ...
  install: async (sitePath: string) => {
    ...
    pkg.scripts["test:visual"] =
      "node skills/community/browser-automation/test-runner.js --action=screenshot";
    pkg.scripts["test:e2e"] =
      "node skills/community/browser-automation/test-runner.js --action=full-test";
    pkg.scripts["test:ci"] =
      "node skills/community/browser-automation/test-runner.js --url=http://localhost:3000";

    fs.writeFileSync(pkgPath, JSON.stringify(pkg, null, 2));

    console.log("✅ Scripts agregados a package.json");
    console.log(
      "📦 Instala Puppeteer: npm install puppeteer --legacy-peer-deps",
    );
    console.log("");
    console.log("🚀 Uso:");
