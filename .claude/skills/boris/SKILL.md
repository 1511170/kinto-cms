...
## Context Map — KINTO CMS (este repo)

Esta es una copia vendorizada de la skill Boris dentro de `kinto-cms`. Cuando el
trabajo es sobre KINTO CMS, usa primero este mapa: liga cada fase del workflow KINTO
con el tip de Boris correspondiente. La metodología KINTO (`AGENTS.md` → "Principios
de Oro") es una adaptación directa de estos tips.

| Fase del workflow KINTO                        | Sección Boris              | Aplicación en KINTO                                                                            |
| ---------------------------------------------- | -------------------------- | ---------------------------------------------------------------------------------------------- |
| Antes de tocar 3+ archivos                     | 3 (Plan Mode)              | `AGENTS.md`: "Planifica antes de tocar 3+ archivos" — entra en plan mode                       |
| `kinto create-site` / `kinto start`            | 37 (Setup Scripts)         | El wizard de arranque ES un setup script — un comando deja todo corriendo                      |
| `kinto skill create`                           | 5 (Skills)                 | "Lo que haces 2x/día conviértelo en skill" — site-skill reutilizable, nunca código ad-hoc      |
| `kinto build` / `kinto deploy`                 | 12, 14 (Verification)      | "Verificación es todo" — build + preview obligatorios antes de entregar                        |
| Corregir un anti-patrón                        | 4 (CLAUDE.md)              | "Cuando algo sale mal, agrégalo a la doc" — actualiza la tabla de Anti-Patrones de `AGENTS.md` |
| Mapear el repo / arquitectura                  | 1, 6 (Parallel/Subagents)  | Usa la agent-skill `graphify` o subagentes Explore en paralelo                                 |
| Revisar un PR de skill                         | 32 (Code Review)           | El marketplace valida PRs con CI (`.github/workflows/skill-ci.yml`)                            |
| Tarea larga (enrichment de catálogo, scraping) | 13, 31 (Long-Running/Loop) | Usa `/loop` para chequeos recurrentes                                                          |
...
16. Git Worktree Support (CLI, Desktop, subagents)
...
_Source: [howborisusesclaudecode.com](https://howborisusesclaudecode.com) - Tips from Boris Cherny's January–April 2026 threads_
